...
function airtelPayload(details: PaymentDetails, phoneNumber: string): Record<string, unknown> {
...
/**
 * Airtel Money payment modal — mirrors MpesaPaymentModal/MTNMoMoPaymentModal's push-then-poll
 * shape (Airtel's "request to pay" is the same push-prompt UX).
 */
export function AirtelMoneyPaymentModal({
  details,
  onClose,
  onSuccess,
  embed = false,
  ...
  // Uganda MSISDNs (256XXXXXXXXX); left permissive for other Airtel markets.
  ...
  const markSuccess = useCallback((receipt?: string) => {
    ...
    setOutcome('success');
    if (embed) {
      sendToParent({ type: 'treasury:payment_confirmed', intentId: details.intent_id || '', amount: details.amount, reference: receipt || details.reference_id, channel: 'airtel_money' });
    }
    onSuccess?.({});
  ...
  const pollStatus = useCallback(async (): Promise<'pending' | 'success' | 'failed'> => {
    ...
    try {
      ...
      if (data.status === 'failed' || data.status === 'cancelled') {
        markFailed(data.message || 'The Airtel Money payment was not completed. Please try again.');
        return 'failed';
      }
    } catch {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (normalized.length < 12) {
      setError('Enter a valid Airtel Money number (e.g. 0701234567).');
      return;
    }
    ...
    try {
      const res = await fetch(details.initiate_url, {
        ...
        body: JSON.stringify(airtelPayload(details, normalized)),
      });
      const data = await res.json().catch(() => ({}));
      if (data.checkout_request_id || data.status === 'processing' || data.status === 'pending') {
        if (embed) sendToParent({ type: 'treasury:payment_initiated', intentId: details.intent_id || '', method: 'airtel_money' });
        onSuccess?.(data);
        setError('');
      ...
      setError(data.message || data.error || 'Could not send Airtel Money prompt. Please try again.');
    } catch {
      ...
      <PaymentModal title="Payment received" onClose={onClose} embed={embed}>
        <div className="space-y-4 text-center py-4">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-muted-foreground">Your Airtel Money payment of {formatAmount()} has been received.</p>
          </div>
      ...
      <PaymentModal title="Payment not completed" onClose={onClose} embed={embed}>
        <div className="space-y-4 text-center py-4">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-muted-foreground">{outcomeMsg || 'The Airtel Money payment was not completed.'}</p>
          </div>
  ...
  if (requestSent) {
    return (
      <PaymentModal title="Pay with Airtel Money" onClose={onClose} embed={embed}>
        <div className="space-y-4 text-center py-2">
          <Loader2 className="h-8 w-8 animate-spin text-primary mx-auto" />
          <div className="space-y-1">
            <p className="text-sm font-semibold text-foreground">Check your phone</p>
            <p className="text-xs text-muted-foreground">
              We sent an Airtel Money prompt to {normalizePhone(phone)}. Approve it on your
  ...
  return (
    <PaymentModal title="Pay with Airtel Money" onClose={onClose} embed={embed}>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {error && <p className="text-sm text-destructive">{error}</p>}
        <p className="text-xs text-muted-foreground">You will receive an Airtel Money prompt on your phone.</p>
        <div className="flex gap-2 justify-end pt-2">
          <Button type="button" variant="outline" onClick={onClose}>Cancel</Button>
          <Button type="submit" disabled={loading}>
            {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : null}
            {loading ? 'Sending…' : `Pay ${formatAmount()} with Airtel Money`}
          </Button>
        </div>
      </form>
    </PaymentModal>
