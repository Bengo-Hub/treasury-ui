    ...
    <div className="space-y-6">
      <div className="rounded-lg border p-4 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div><label className="text-xs text-muted-foreground">Period (YYYYMM)</label><input className="w-full rounded border px-3 py-2 text-sm" placeholder="202406" value={totForm.period} onChange={(e) => setTotForm({ ...totForm, period: e.target.value })} /></div>
          <div><label className="text-xs text-muted-foreground">Turnover Amount</label><input type="number" className="w-full rounded border px-3 py-2 text-sm" value={totForm.turnover} onChange={(e) => setTotForm({ ...totForm, turnover: e.target.value })} /></div>
          <div><label className="text-xs text-muted-foreground">Tax Amount</label><input type="number" className="w-full rounded border px-3 py-2 text-sm" value={totForm.taxAmount} onChange={(e) => setTotForm({ ...totForm, taxAmount: e.target.value })} /></div>
          <div><label className="text-xs text-muted-foreground">Obligation Code</label><input className="w-full rounded border px-3 py-2 text-sm" value={totForm.obligationCode} onChange={(e) => setTotForm({ ...totForm, obligationCode: e.target.value })} /></div>
        </div>
        <button className="rounded bg-primary px-4 py-2 text-sm text-primary-foreground disabled:opacity-50" disabled={!totForm.pin || !totForm.period || fileTOT.isPending} onClick={handleTOT}>
