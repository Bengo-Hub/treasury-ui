...
import { createCRMContact, updateCRMContact, crmContactDisplayName, type CRMContact } from '@/lib/api/crm';
...
interface Props {
  ...
  initialName?: string;
  /**
   * When set, the modal opens in edit mode against this existing CRM contact (fetched here so
   * the form hydrates from its live/full record) instead of creating a new one. Callers should
   * pass this whenever the picked/linked customer already exists — see crmCustomerId in
   * SharedDocumentCreateView — rather than routing back through the create flow, which would
   * otherwise leave the user re-typing details into a brand-new duplicate contact.
   */
...
/**
 * Inline "create or edit a customer" dialog for the invoice/quotation "Billed To" picker —
 ...
 * via the existing createCRMContact()/updateCRMContact()/proxy routes. MarketFlow's Contact
 * schema has no dedicated address/company/tax-pin columns, so anything beyond name/email/phone
 * is carried in Contact.metadata — a JSON catch-all field that already exists, so this needs no
 * marketflow-api schema migration.
 */
export function CreateClientModal({ tenant, initialName = '', editContactId, onCreated, onClose }: Props) {
  const isEdit = !!editContactId;
  const { data: existingContact, isLoading: contactLoading, isError: contactLoadError } =
    useCRMContact(tenant, editContactId, isEdit);
  ...
  // Hydrate the form once the live contact loads — adjusted during render (React's documented
  // pattern: https://react.dev/learn/you-might-not-need-an-effect#adjusting-some-state-when-a-prop-changes)
  // rather than in a useEffect, which would cause an extra avoidable render/commit and is what
  // the lint's `set-state-in-effect` rule flags (see platform/payouts/page.tsx for the same
  // convention elsewhere in this codebase). Keyed on the contact id so a background refetch of
  // the SAME contact never clobbers edits the user has already started typing.
  const [hydratedContactId, setHydratedContactId] = useState<string | undefined>(undefined);
  if (isEdit && existingContact && existingContact.id !== hydratedContactId) {
    setHydratedContactId(existingContact.id);
    const meta = (existingContact.metadata ?? {}) as Record<string, unknown>;
    setForm({
      name:           crmContactDisplayName(existingContact),
  ...
  const mutation = useMutation({
    mutationFn: () => {
      ...
      if (isEdit && editContactId) {
        // Start from the contact's OWN existing metadata (not just the fields this form tracks)
        // and layer edits on top — updateCRMContact/marketflow replaces Metadata wholesale, so
        // any untracked key (set by another system) would otherwise be silently wiped.
        const metadata: Record<string, unknown> = { ...(existingContact?.metadata ?? {}) };
        const setOrClear = (key: string, value: string) => {
          const v = value.trim();
          if (v) metadata[key] = v; else delete metadata[key];
        };
        setOrClear('contact_person', form.contact_person);
        setOrClear('kra_pin', form.kra_pin);
        setOrClear('address', form.address);
        setOrClear('country', form.country);
        setOrClear('notes', form.notes);

        return updateCRMContact(tenant, editContactId, {
          // Sent as plain strings (not `|| undefined`) so clearing a field in the UI actually
          // clears it server-side — an omitted key means "leave unchanged", not "blank it out".
          first_name: first,
          last_name:  rest.join(' '),
  ...
  const loadingExisting = isEdit && contactLoading && !hydratedContactId;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border sticky top-0 bg-card">
          <h2 className="text-sm font-black text-foreground">{isEdit ? 'Edit Client' : 'Add New Client'}</h2>
        ...
        {loadingExisting ? (
          ...
          <>
            ...
            <div className="p-6 space-y-4">
              {isEdit && contactLoadError && (
                <p className="text-xs text-destructive font-semibold">
                  Couldn&apos;t load the latest client details — showing what we already have. You can still save changes.
                </p>
              )}
              {/* Name */}
              <div>
            ...
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-border bg-accent/20 sticky bottom-0">
              ...
              <button type="button" disabled={!form.name.trim() || mutation.isPending}
                ...
                {mutation.isPending ? (isEdit ? 'Saving…' : 'Creating…') : (isEdit ? 'Save Changes' : 'Create & Use Client')}
              </button>
            </div>
          </>
        )}
