...
export function PaymentsTab({
  ...
  useEffect(() => {
    if (payoutConfig) {
      ...
      if (savedCurrency) setPayoutCurrency(savedCurrency);
    }
  }, [payoutConfig]);
  ...
  return (
    <div className="space-y-6">
      {/* Max payment amount (from original settings) */}
      <Card>
        <CardContent className="p-6 space-y-4">
          <div className="flex items-center gap-2 mb-2">
            <CreditCard className="h-4 w-4 text-primary" />
            <h3 className="font-bold text-sm uppercase tracking-tight">Payment Limits</h3>
          </div>
          <div>
            <label className="block text-xs font-medium text-muted-foreground mb-1">Max Payment Amount</label>
            <p className="text-xs text-muted-foreground mb-2">Maximum single payment amount allowed (0 = no limit).</p>
            <div className="flex items-center gap-3">
              ...
              />
              <Button size="sm" className="gap-2" disabled={isSavingSettings} onClick={onSaveMaxAmount}>
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="font-bold text-sm uppercase tracking-tight text-muted-foreground">Payment Gateways</h3>
                  <p className="text-xs text-muted-foreground mt-1">
                    Activate gateways to show them on your pay pages. Set one as primary — it will be the default for customers.
                  </p>
                </div>
                {isSuperAdmin && (
                  <Link href={`/${orgSlug}/platform`}>
                    <Button size="sm" variant="outline" className="gap-1.5 shrink-0">
                      <Plus className="h-3.5 w-3.5" /> Platform config
                    </Button>
                  </Link>
                )}
              </div>
                  ...
                  return (
                    ...
                    >
                      <div className={cn(
                        ...
                        isMpesaGw ? 'bg-green-500/10' : 'bg-blue-500/10',
                      )}>
                      ...
                      <div className="flex items-center gap-2">
                        {isActive && !isPrimary && (
                          <Button
                            ...
                            onClick={() => selectMutation.mutate(gw.gateway_type, { onSuccess: () => toast.success(`${gw.name} set as primary`) })}
                        ...
                        {isActive ? (
                          <div className="flex items-center gap-1.5">
                            ...
                            <Button
                              ...
                              onClick={() => deactivateMutation.mutate(gw.gateway_type, {
                          ...
                          <Button
                            size="sm"
                            disabled={selectMutation.isPending}
                            onClick={() => selectMutation.mutate(gw.gateway_type, { onSuccess: () => toast.success(`${gw.name} activated`) })}
                          >
                            {selectMutation.isPending
                              ? <Loader2 className="h-3.5 w-3.5 animate-spin" />
                              : <span className="flex items-center gap-1.5"><Power className="h-3.5 w-3.5" /> Activate</span>}
                          </Button>
                        )}
                      </div>
                    </div>
                        ...
                        <>
                          ...
                          >
                              ...
                              <>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                  <div>
                                    ...
                                    <select
                                      ...
                                      onChange={(e) => {
                                        ...
                                        setPayoutForm((f) => ({ ...f, bank_code: e.target.value, bank_name: selectedBank?.name ?? '' }));
                                        setVerifiedName(null);
                                        setVerifyError(null);
                                      }}
                                  ...
                                  <div>
                                    ...
                                    <input value={payoutForm.bank_name} readOnly className="w-full rounded-lg border border-input bg-muted px-3 py-2 text-sm" />
                                  </div>
                                  ...
                                  <div>
                                          ...
                                          resolveAccountMutation.mutate(
                                            { accountNumber: payoutForm.account_number, bankCode: payoutForm.bank_code },
                                            {
                                              onSuccess: (res: any) => {
                                                ...
                                                if (name) { setVerifiedName(name); setPayoutForm((f) => ({ ...f, account_name: name })); }
                                                else setVerifyError('Could not resolve account name');
                                              },
                                              onError: (err: any) => setVerifyError(err?.response?.data?.message || err?.message || 'Verification failed'),
                                            },
                                          );
                                        }}
                                        className="shrink-0"
                                    ...
                                    {verifiedName && <p className="mt-1 flex items-center gap-1 text-xs text-green-600"><CheckCircle2 className="h-3.5 w-3.5" /> {verifiedName}</p>}
                                    {verifyError && <p className="mt-1 flex items-center gap-1 text-xs text-destructive"><XCircle className="h-3.5 w-3.5" /> {verifyError}</p>}
                                  </div>
                                  ...
                                  <div>
                                    ...
                                    <input
                                      ...
                                      className={cn('w-full rounded-lg border border-input px-3 py-2 text-sm', verifiedName ? 'bg-green-50 border-green-300 dark:bg-green-950/20 dark:border-green-800' : 'bg-background')}
                                    />
                                  </div>
                                </div>
                              </>
                            ...
                            {payoutForm.recipient_type === 'mobile_money' && (
                              <div>
                                ...
                                <input value={payoutForm.mobile_number} onChange={(e) => setPayoutForm((f) => ({ ...f, mobile_number: e.target.value }))} className="w-full max-w-xs rounded-lg border border-input bg-background px-3 py-2 text-sm" placeholder="e.g. +254712345678" />
                              </div>
                            )}
                            ...
                            {payoutForm.recipient_type === 'mobile_money_business' && (
                              <div>
                                ...
                                <input value={payoutForm.account_number} onChange={(e) => setPayoutForm((f) => ({ ...f, account_number: e.target.value }))} className="w-full max-w-xs rounded-lg border border-input bg-background px-3 py-2 text-sm" placeholder="e.g. 174379" />
                              </div>
                            )}
                            ...
                            {payoutForm.recipient_type === 'mpesa_paybill' && (
                              <div>
                                ...
                                <input value={payoutForm.mpesa_paybill} onChange={(e) => setPayoutForm((f) => ({ ...f, mpesa_paybill: e.target.value }))} className="w-full max-w-xs rounded-lg border border-input bg-background px-3 py-2 text-sm" />
                              </div>
                            )}
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                              <div>
                                ...
                                <select value={payoutForm.schedule_type} onChange={(e) => setPayoutForm((f) => ({ ...f, schedule_type: e.target.value, schedule_day: 1 }))} className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm">
                              ...
                              {payoutForm.schedule_type === 'weekly' && (
                                <div>
                                  ...
                                  <select value={payoutForm.schedule_day} onChange={(e) => setPayoutForm((f) => ({ ...f, schedule_day: parseInt(e.target.value, 10) }))} className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm">
                                    {WEEKDAYS.map((day, idx) => <option key={day} value={idx + 1}>{day}</option>)}
                                  </select>
                                </div>
                              )}
                          ...
                          {payoutConfig && (
                            <div className="mt-6 pt-4 border-t flex flex-wrap gap-3">
                              <div className="rounded-lg bg-muted px-3 py-2 text-center min-w-24">
                      ...
                      {loadingMpesa ? (
                        ...
                        >
                          ...
                          <div>
                            ...
                            <input value={mpesaForm.initiator_name} onChange={(e) => setMpesaForm((f) => ({ ...f, initiator_name: e.target.value }))} placeholder="Leave empty to keep current value" className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" />
                          </div>
                          <div>
                            <label className="block text-xs font-medium text-muted-foreground mb-1">Initiator password</label>
                            <input type="password" value={mpesaForm.initiator_password} onChange={(e) => setMpesaForm((f) => ({ ...f, initiator_password: e.target.value }))} placeholder="Leave blank to keep current" className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" autoComplete="new-password" />
                          </div>
