 ...
 *   proforma_invoice, sales_order, credit_note, debit_note, payment_receipt
...
export function SharedInvoiceCreateView({ effectiveTenant, docType, onClose, editId }: Props) {
  ...
  const handleSave = useCallback(() => {
    ...
    if (isEdit && editId) {
      updateMutation.mutate(base as UpdateInvoiceRequest, { onSuccess: onClose });
    } else {
      createMutation.mutate({ ...base, invoice_type: config.invoiceType } as CreateInvoiceRequest, { onSuccess: onClose });
    }
  }, [form, lines, customerId, isEdit, editId, config, createMutation, updateMutation, onClose]);
    ...
    <div className="min-h-screen pb-24 bg-background">
      <div className="sticky top-0 z-50 border-b border-border px-6 py-4 flex items-center justify-between shadow-sm bg-card">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-base font-black text-foreground tracking-tight">{isEdit ? `Edit ${config.title}` : `Create ${config.title}`}</h1>
          ...
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 pb-6 border-b border-border">
            <div className="space-y-4 w-full max-w-sm">
              <div className={cn('grid gap-4', config.showDueDate ? 'grid-cols-2' : 'grid-cols-1')}>
                <div>
                  <label className="text-xs font-bold block mb-1 text-foreground">{config.primaryDateLabel}<span className="text-destructive">*</span></label>
                  <input type="date" className={inputCls} value={form.invoice_date}
                    onChange={e => setForm(p => ({ ...p, invoice_date: e.target.value }))} />
                </div>
                {config.showDueDate && (
                  <div>
                    <label className="text-xs font-bold block mb-1 text-foreground">{config.secondaryDateLabel}<span className="text-destructive">*</span></label>
